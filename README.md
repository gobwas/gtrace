...
Traces can be nested: a trace structure may contain fields of other trace types. Lets say our `Client` also owns a connection pool with its own trace points, and we want to expose them as part of `ClientTrace`:
...
type PoolTrace struct {
	OnAcquire func() func(net.Conn, error)
}
...
For such fields the generated `Compose()` calls `Compose()` of the nested trace, so nested _hooks_ are merged the same way as the top level ones:
...
t = t.Compose(ClientTrace{
	Pool: PoolTrace{
		OnAcquire: func() func(net.Conn, error) {
			log.Println("acquire start")
			return func(_ net.Conn, err error) {
				log.Printf("acquire done; err=%v", err)
			}
		},
	},
})
	...
	Pool: PoolTrace{
		OnAcquire: func() func(net.Conn, error) {
			...
			return func(net.Conn, error) {
				sendAcquireLatency(time.Since(start))
			}
		},
	},
	OnPing: func() func(error) {
		// Only the second trace is interested in pings.
		return func(err error) {
			log.Printf("ping done; err=%v", err)
		}
	},
})
// t.Pool.OnAcquire now calls both hooks; t.OnPing calls the single one.
```

A field is treated as a nested trace if its type is another `//gtrace:gen` structure from the same package, or a type from another package which has a `Compose()` method with the same shape (that is, generated by **gtrace** as well).

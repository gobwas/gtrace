...
func (w *Writer) Write(p Package) error {
	...
	if expr := buildConstraint(p.BuildConstraint, w.BuildTag, w.Stub); expr != nil {
		w.line()
		w.line(`//go:build `, expr.String())
	}
	w.line()
	...
	w.newScope(func() {
		for i, trace := range p.Traces {
			if i > 0 {
				w.line()
			}
			w.compose(trace)
			if trace.Nested {
				// isZero has a doc comment, which requires a blank line
				// after the preceding declaration.
				w.line()
				w.isZero(trace)
			}
...
// buildConstraint returns the build constraint of the generated file: the
// constraint of the source file (if any) combined with the build tag given to
// gtrace (if any). It returns nil if the file must not be constrained.
func buildConstraint(src constraint.Expr, tag string, stub bool) constraint.Expr {
	if tag == "" {
		return src
	}
	var t constraint.Expr = &constraint.TagExpr{Tag: tag}
	if stub {
		t = &constraint.NotExpr{X: t}
	}
	if src == nil {
		return t
	}
	return &constraint.AndExpr{X: src, Y: t}
}
...
func (w *Writer) stubTrace(id string, t *Trace) (name string) {
	...
	name = w.declare(name)
	// A var declaration between func declarations must be separated by
	// blank lines to keep the output gofmt-clean.
	w.line()
